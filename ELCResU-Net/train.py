...
# %%
for index in range(len(kernel_size)):
    model = LCResUNet(1, 3, kernel_size=kernel_size[index]).to(device)
    ...
    for epoch in range(epochs):
        ...
        if val_loss < running_best_acc.get_best():
            ...
            torch.save(model.state_dict(), os.path.join('./trained_model', f"model_JNU_{index+1}.pt"))
            print("Save model.")
