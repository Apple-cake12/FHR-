...
from signal_processing import read_ctg_from_csv, preprocess_signal, apply_butterworth_filter, get_baseline, \
    get_accident, segment_signal_with_overlap, splice_segments
...
def visual_acc_dec(fhr,ground_labels,pred_labels,ground_baseline,filename,save_dir=r'.\acc_dec_results'):
    ...
    pred_cls = torch.argmax(pred_labels, dim=1).detach().cpu().squeeze().numpy().copy()  # (32,4800)  # Predicted labels
...
model.load_state_dict(torch.load('./trained_model/model_GMU.pt'))
model.eval()
