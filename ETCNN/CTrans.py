...
import numpy as np
from torch.nn import Dropout, Softmax, Conv1d, LayerNorm
...
class Channel_Embeddings(nn.Module):
    ...
    def _init_weights(self):
        torch.nn.init.kaiming_normal_(self.patch_embeddings.weight, a=0, mode='fan_out', nonlinearity='relu')

    def forward(self, x):
        if x is None:
            return None
        x = self.patch_embeddings(x)  # (B, hidden. n_patches^(1/2), n_patches^(1/2)) Conv1d 对特征图进行卷积操作，将特征图划分为多个 patch
        x = x.flatten(2)
...
class Attention_org(nn.Module):
    def __init__(self, config, vis,channel_num):
        ...
        self.vis = vis
        # KV_size固定, 若channel_num=[8, 16, 32, 64, 128]，
        # KV_size = 8+16+32+64+128=248
...
class ChannelTransformer(nn.Module):
    def __init__(self, config, vis, img_size, channel_num=[8, 16, 32, 64, 128], patchSize=[32, 16, 8, 4, 2]):
        super().__init__()
        ...
        self.reconstruct_1 = Reconstruct(channel_num[0], channel_num[0], kernel_size=1,scale_factor=self.patchSize_1)
        self.reconstruct_2 = Reconstruct(channel_num[1], channel_num[1], kernel_size=1,scale_factor=self.patchSize_2)
        self.reconstruct_3 = Reconstruct(channel_num[2], channel_num[2], kernel_size=1,scale_factor=self.patchSize_3)
        self.reconstruct_4 = Reconstruct(channel_num[3], channel_num[3], kernel_size=1,scale_factor=self.patchSize_4)
        self.reconstruct_5 = Reconstruct(channel_num[4], channel_num[4], kernel_size=1,scale_factor=self.patchSize_5)
    ...
    def forward(self,en1,en2,en3,en4,en5):
        ...
        #每个输入特征图首先通过一个通道嵌入模块（Channel_Embeddings），该模块将特征图划分为多个“patch”，并为每个 patch 添加位置嵌入（position embeddings）。这一步类似于 Vision Transformer 中的 patch embedding，但专门用于处理通道信息。
