...
class IOU(nn.Module):
    ...
    def forward(self, preds, labels):
        ...
        # labels: (B, L) 类别索引
        num_classes = preds.shape[1]
        preds = torch.argmax(preds, dim=1)  # (B, L)
        labels = labels.long()
...
def mstatscompare_P(fhr,ground_labels,pred_labels,ground_baseline):
    ...
    # 保持 pred_cls 和 ground_cls 为张量（用于指标计算）
    pred_cls = torch.argmax(pred_labels, dim=1).detach().cpu().squeeze()  # (B, L) 保持为张量
    ground_cls = ground_labels.detach().cpu().squeeze()  # (B, L) 保持为张量
    ...
    #加权F1系数
    stats['F1'] = f1_score(ground_cls,pred_cls,average='weighted')
    #Kappa系数
    stats['Kappa'] = cohen_kappa_score(ground_cls,pred_cls)

    # 初始化指标类
    Index_Recall_Base = ClassWiseRecall(class_idx=0)
    Index_Recall_Acc = ClassWiseRecall(class_idx=1)
    Index_Recall_Dec = ClassWiseRecall(class_idx=2)
    Index_MacroF1 = MacroF1()
    Index_MCC = MatthewsCorrCoef()
    Index_GMean = GeometricMean()
    # ------------------- 指标计算（使用张量） -------------------
    stats['Recall_Base'] = Index_Recall_Base(pred_cls, ground_cls).cpu().numpy()
    stats['Recall_Acc'] = Index_Recall_Acc(pred_cls, ground_cls).cpu().numpy()
    stats['Recall_Dec'] = Index_Recall_Dec(pred_cls, ground_cls).cpu().numpy()
    stats['MacroF1'] = Index_MacroF1(pred_cls, ground_cls).cpu().numpy()
    stats['MCC'] = Index_MCC(pred_cls, ground_cls).cpu().numpy()
    stats['GMean'] = Index_GMean(pred_cls, ground_cls).cpu().numpy()
    # ------------------- 信号后处理（转换为 NumPy） -------------------
    # 将 pred_cls 转换为 NumPy 用于基线估计
    pred_cls_np = pred_cls.numpy().copy()  # (B, L) 转换为 NumPy 数组
    #基线处理
    pred_baselines = get_baseline_P(FHR,pred_cls_np,4800,240,480)
    #MAE指标
    ...
    LDB1 = ground_baselines.copy()
    LDB2 = pred_baselines.copy()
...
# ------------------- 逐类召回率 -------------------
class ClassWiseRecall(nn.Module):
    def __init__(self, class_idx, smooth=1e-6):
        super(ClassWiseRecall, self).__init__()
    ...
    def forward(self, preds, targets):
        ...
        # 使用 PyTorch 布尔操作
        pred_class = (preds == self.class_idx)
        target_class = (targets == self.class_idx)

        TP = torch.sum(pred_class & target_class).float()  # 正确预测
        FN = torch.sum(target_class) - TP                  # 漏检
...
class MacroF1(nn.Module):
    ...
    def forward(self, preds, targets):
        ...
        for class_idx in range(num_classes):
            ...
            TP = torch.sum(pred_class & target_class).float()
            FP = torch.sum(pred_class) - TP
            FN = torch.sum(target_class) - TP
...
# ------------------- Matthews相关系数 -------------------
class MatthewsCorrCoef(nn.Module):
    ...
    def forward(self, preds, targets):
        conf_matrix = torch.zeros((self.num_classes, self.num_classes), device=preds.device)
...
class GeometricMean(nn.Module):
    ...
    def forward(self, preds, targets):
        ...
        return torch.exp(torch.mean(torch.log(torch.stack(recalls) + self.smooth)))